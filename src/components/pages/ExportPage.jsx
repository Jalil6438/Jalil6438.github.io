...
import { RECOVERY_UI_ENABLED } from "../../recovery/recoveryClient.js";

export default function ExportPage({ dark, onBack, onExport, onImport, onRecovery }) {
  return (
    <AppPage dark={dark} title="Backup & Restore" subtitle="Save or restore your progress" onBack={onBack}>
      ...
      {RECOVERY_UI_ENABLED && <>
        <div style={{ height: 1, margin: "22px 0 18px", background: dark ? "rgba(217,177,95,0.15)" : "rgba(0,0,0,0.08)" }} />
        <div style={{ fontSize: 13, color: dark ? "rgba(243,231,200,0.75)" : "#3D2E0A", lineHeight: 1.7, marginBottom: 14 }}>
          Review a verified recovery snapshot before applying any changes.
        </div>
        <button type="button" className="sbtn" onClick={onRecovery} style={{
          width: "100%", padding: "13px 16px", borderRadius: 8, textAlign: "center",
          fontSize: 13, fontWeight: 700, background: "transparent",
          color: dark ? "#E0BD78" : "#8A6A10",
          border: `1px solid ${dark ? "rgba(217,177,95,0.40)" : "rgba(140,100,20,0.35)"}`,
          cursor: "pointer",
        }}>View progress recovery</button>
      </>}
    </AppPage>
  );
}
