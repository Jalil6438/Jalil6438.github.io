...
import { JUZ_SURAHS, JUZ_META } from "../data/quran-metadata";
        ...
        {juzGroups.map(g => (
          <div key={g.juz} style={{marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6,padding:"0 2px"}}>
              <div style={{fontSize:10,color:dark?"rgba(217,177,95,0.55)":"rgba(140,100,20,0.60)",letterSpacing:".14em",textTransform:"uppercase",fontWeight:700}}>{(()=>{const r=JUZ_META.find(m=>m.num===g.juz)?.roman;return r?`Juz ${g.juz} · ${r}`:`Juz ${g.juz}`;})()}</div>
