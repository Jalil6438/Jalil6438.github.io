        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:parchment}}>
          ...
          {drawerView!=="tafsir"&&(quranMode==="mushaf"?(
            ...
            <div
              ...
              style={{position:"relative",flex:1,overflowY:"auto",background:dark?"linear-gradient(180deg,#0B1220,#0E1628)":"#F3E9D2",padding:`4px 6px ${haramainMeta?"80px":"8px"}`,display:"flex",flexDirection:"column",justifyContent:"flex-start"}}
            >
              ...
              {mushafLoading?(
                ...
                <div style={mushafPage<=2?{padding:0,flex:1,display:"flex",flexDirection:"column",minHeight:0}:{padding:0}}>
                    ...
                    return (<div style={mushafPage<=2?{padding:"8px 2px 0",position:"relative",flex:1,display:"flex",flexDirection:"column",minHeight:0}:{padding:"8px 2px 0",position:"relative"}}>
                      {(()=>{
                        ...
                        const entries=pageLayout.map((layoutEntry,i)=>{
                          ...
                          return (
                          <div key={i} className={vkForLine?"sbtn":undefined} onClick={vkForLine?()=>{setSelectedAyah(vkForLine);setDrawerView("default");setShowPickers(true);setTimeout(()=>{try{window.scrollTo({top:0,behavior:"smooth"});document.querySelectorAll('[class*="fi"]').forEach(el=>{if(el.scrollTop>0)el.scrollTo({top:0,behavior:"smooth"});});}catch{}},10);}:undefined} style={{direction:"rtl",display:"flex",justifyContent:isCenter?"center":"space-between",alignItems:"center",maxWidth:"min(560px,94vw)",marginInline:"auto",fontFamily:`'p${mushafPage}',serif`,fontSize:"clamp(18px,4.6vw,26px)",color:dark?"#E8DFC0":"#2D2A26",padding:"1px 0",whiteSpace:"nowrap",gap:isCenter?"0.25em":0,cursor:vkForLine?"pointer":"default"}}>
                  ...
                  {(()=>{
                    ...
                    return (
                      <div style={{marginTop:6,textAlign:isOdd?"right":"left",padding:isOdd?"0 20px 0 0":"0 0 0 20px",fontFamily:"'IBM Plex Mono',monospace",fontSize:12,color:dark?"rgba(217,177,95,0.60)":"#6B645A",letterSpacing:".06em"}}>
                        {text}
                      </div>
                    );
                  })()}
