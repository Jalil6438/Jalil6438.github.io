...
import buildGlobalCss from "./styles/globalCss";
...
export default function RihlatAlHifz() {
  ...
  const fetchSimVerse=async(vk)=>{
    ...
    // look-alike + its neighbours, cached by plain verse_key
  ...
  if(isDhuhr){
    ...
    if(allJuzVerses.length>0){
      ...
      if(true){
        ...
        // Today's filter excludes only today's specific verse_keys, not
        // the whole V2 page. A page like 586 holds today's Abasa 41-42
        // AND prior-day Takwir 1-X — page-based exclusion would drop the
        // prior memorization too. Verse-key exclusion is precise: today's
        // verses get filtered, prior verses on the same page stay in.
        ...
        allJuzVerses.forEach((v,i)=>{
          ...
          v.page_number=vPage;
          if(v.verse_key&&todayVerseKeys.has(v.verse_key)) return;
  ...
  return (
    <div className={dark?"":"lm"} style={{fontFamily:"'DM Sans',sans-serif",background:T.bg,height:"100dvh",minHeight:"100vh",color:T.text,display:"flex",flexDirection:"column",transition:"background .25s,color .25s",position:"relative",overflow:"hidden"}}>
      ...
      {!showOnboarding&&showDua&&<DuaScreen dark={dark} duaIdx={duaIdx} setShowDua={setShowDua} setDuaIdx={setDuaIdx} setActiveTab={setActiveTab} />}
      ...
      <BottomTabBar
        activeTab={activeTab} appPage={appPage} dark={dark}
        setActiveTab={setActiveTab} setRihlahTab={setRihlahTab}
      />
      ...
      {!appPage&&activeTab==="rihlah"&&rihlahTab==="timeline"&&(
        <PlanTimeline dark={dark} setRihlahTab={setRihlahTab} rihlahScrollRef={rihlahScrollRef} userPlanMode={userPlanMode} goalYears={goalYears} goalMonths={goalMonths} goalLabel={goalLabel} dailyNew={dailyNew} timeline={timeline} pct={pct} sessionJuz={sessionJuz} />
      )}
    ...
    {badgeCelebration&&(()=>{
      ...
      <div onClick={advance} style={{position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.60)",backdropFilter:"blur(4px)"}}>
        <div style={{background:dark?"linear-gradient(180deg,#0E1628 0%,#080E1A 100%)":"#EADFC8",borderRadius:24,padding:"32px 28px",maxWidth:320,width:"90%",textAlign:"center",border:"1px solid rgba(217,177,95,0.25)",boxShadow:"0 20px 60px rgba(0,0,0,0.50),0 0 40px rgba(212,175,55,0.15)"}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{fontSize:13,color:dark?"rgba(243,231,200,0.65)":"#6B645A",lineHeight:1.6,marginBottom:10}}>{badgeCelebration.message}</div>
          {badgeCelebration.dua&&(<>
            <div style={{fontFamily:"'UthmanicHafs','Amiri Quran','Amiri',serif",fontSize:30,color:dark?"#F6E27A":"#B45309",direction:"rtl",lineHeight:1.6,marginTop:0,marginBottom:2,wordSpacing:"0.25em",padding:"0 6px"}}>{badgeCelebration.dua.ar}</div>
            <div style={{fontSize:13,fontStyle:"italic",color:dark?"rgba(243,231,200,0.65)":"#8B7355",marginBottom:18}}>"{badgeCelebration.dua.en}"</div>
          </>)}
