...
function matchesApprovedRecord(raw) {
  let state;
  try { state = JSON.parse(raw); } catch { return false; }
  if (!state || typeof state !== "object" || Array.isArray(state)
    || !Array.isArray(state.snapshots) || !Array.isArray(state.restoreOperations)
    || state.snapshots.length !== 1 || state.restoreOperations.length !== 0) return false;
  const envelope = state.snapshots[0]?.envelope;
  return envelope?.appVersion === "preview-smoke"
    && envelope?.backupId === "bkup_synthetic_source"
    && envelope?.writerId === "wrtr_synthetic_source";
}
...
export async function remediatePreview({
  ...
  do {
    ...
    for (const key of result[1]) {
      ...
      keys.add(key);
      if (keys.size > maxRecords) throw new Error("scan-bounded");
    }
  } while (cursor !== "0");
  ...
  for (const key of keys) {
    ...
    if (matchesApprovedRecord(raw)) matches.push({ key, raw });
    if (matches.length > 1) throw new Error("multiple-matches");
  }
  if (matches.length === 0) return { scanned: keys.size, deleted: 0 };
